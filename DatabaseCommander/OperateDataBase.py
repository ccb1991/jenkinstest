...
class ImportDataOperater(AbstractDataOperater):
    ...
    # def __init_client_cursor_encode(self):
    #     '''初始化数据库游标'''
    #     curs = self.Connect.cursor()
    #     curs.execute("SET NAMES utf8")
    #     self.Connect.commit()
...
class QueryExportData():
    ...
    def query_condition_date(self, keyname:str, tablename:str):
        '''根据企业id查询‘关键字’数据
        result：返回列表嵌套的字典'''
        sql='select %s from %s where epid=%s' %( keyname,tablename,self.epid)
    ...
    def filter_table_with_eqid(self,):
        ...
        sql = "SELECT DISTINCT TABLE_NAME FROM INFORMATION_SCHEMA.COLUMNS " \
              "where TABLE_SCHEMA ='%s' and COLUMN_NAME='epid'" \
              % (self.DataBase.mysqldbname)
        cursor = self.__init_client_cursor_encode()
        cursor.execute(sql)
        cursor.close()
        eqidtablelist = list(map(lambda tablelist: list(tablelist.values())[0], cursor.fetchall()))
    ...
    def query_data_rownum(self, tablename:str, dbname:str, conditionlist):
        ...
        def query_submeter():
            ...
            cursor.execute(sql)
            tablenamelist=list(map(lambda tablelist:list(tablelist.values())[0],cursor.fetchall()))
            return  tablenamelist
        alltablename=query_submeter()
        targettablenamelist=list(filter(lambda tablist:tablist.startswith(tablename),alltablename))
        totalrowsnum = 0
        for targettablename in targettablenamelist:
            for condition in conditionlist:
                for key,value in condition.items():
                    ...
                    if result != None:
                        totalrowsnum += int(result['count(*)'])
                        resultdict[tablename]=totalrowsnum
                    cursor.close()
        return resultdict
